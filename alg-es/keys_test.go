...
var (
	...
	pkixRSA  = []byte{0x30, 0x20, 0x30, 0xd, 0x6, 0x9, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0xd, 0x1, 0x1, 0x1, 0x5, 0x0, 0x3, 0xf, 0x0, 0x30, 0xc, 0x2, 0x5, 0x0, 0xd6, 0xc8, 0xbc, 0x87, 0x2, 0x3, 0x1, 0x0, 0x1}
)
...
func TestNewSettingsWithKeyURL(t *testing.T) {
	...
	tests := []struct {
		...
		{"PKCS8 RSA key", args{pkcs8RSA, "key_id", "key_url"}, Settings{}, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestProvider_AddPublicKey(t *testing.T) {
	...
	tests := []struct {
		...
		{"PKIX RSA key", &Provider{keys: make(map[string]*ecdsa.PublicKey)}, args{publickey.New(pkixRSA, "key_id")}, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
